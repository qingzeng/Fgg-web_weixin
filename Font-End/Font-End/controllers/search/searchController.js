 ...
 * searchCtrl/小区模糊查询
 */
define(['app', 'jquery', '_layer', 'handler', '../../services/search/searchServices'], function(app, $, $layer, handler, searchServices) {
	app.controller('searchCtrl', function($scope, searchServices) {
		...
		$("#residentialAreaName").on('keyup paste', function() {
			if ($.trim($(this).val()) == "") {
				...
				$("#list").show();
				$scope.formData.residentialAreaName = $.trim($(this).val());
				searched($scope, searchServices);
			}
